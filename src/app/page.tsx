...
function Status({ job, onRetry }: { job: Job; onRetry: () => void }) {
  ...
  return <><div className={`status-dot ${job.status}`} /><h3>{copy[0]}</h3><p>{copy[1]}</p><div className="job-id">Job {job.id.slice(0, 8)}</div>{job.status === "completed" && job.downloadUrl && <><a className="download" href={job.downloadUrl}>Download PDF <span>↓</span></a><a className="secondary-link" href={`/api/reports/${job.id}/data`} target="_blank" rel="noreferrer">View structured output &amp; run trace</a></>}{job.status === "failed" && <button className="retry" onClick={onRetry}>Retry job</button>}</>;
}
