...
export const reportSchema = z.object({
  ...
  highlights: z.array(z.object({ text: z.string().min(1).max(360), evidence: evidenceSchema })).min(1),
  outlook: z.object({ text: z.string().min(1).max(900), evidence: evidenceSchema }),
  companyData: z.array(z.object({ label: z.string().min(1).max(48), value: evidenceValueSchema.optional() })).default([]),
  quarterlyFinancials: z.array(financialMetricSchema).default([]),
  annualFinancials: z.array(z.object({
    metric: z.string().min(1).max(60),
    values: z.array(z.object({ period: z.string().min(1).max(16), value: evidenceValueSchema.optional() })).default([]),
  })).default([]),
