...
function db() {
  if (!_db) {
    ...
    _db = new DatabaseSync(path.resolve(config.storageRoot, "bull-ai.sqlite"));
    _db.exec(`
      CREATE TABLE IF NOT EXISTS report_jobs (
        ...
        error TEXT, report_path TEXT, report_json TEXT, trace_json TEXT,
...
const mapJob = (row: Record<string, unknown>): ReportJob => ({
  ...
  reportJson: row.report_json ? String(row.report_json) : null,
  traceJson: row.trace_json ? String(row.trace_json) : null,
...
export function completeJob(id: string, reportPath: string, reportJson: string, traceJson: string) {
  db().prepare("UPDATE report_jobs SET status='completed', report_path=?, report_json=?, trace_json=?, error=NULL, updated_at=? WHERE id=?")
    .run(reportPath, reportJson, traceJson, new Date().toISOString(), id);
}
